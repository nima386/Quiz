...
function startFocusGame(key) {
  if (key === "europe") startEuropeMapQuiz();
  if (key === "asia") startAsiaMapQuiz();
  if (key === "africa") startAfricaMapQuiz();
  if (key === "southAmerica") startSouthAmericaMapQuiz();
...
async function initGamesGlobeFinal() {
  ...
  if (typeof Globe !== "function") {
    ...
    gamesGlobeRetryTimer = setTimeout(initGamesGlobeFinal, 700);
    return;
  }
...
const MapQuizEngine = (() => {
  ...
  function clearMap(config) {
    byId(config.mapId)?.querySelectorAll("[data-country]").forEach(land => {
      land.classList.remove(
        ...answeredClasses,
        "temp-wrong-country",
        "country-flash",
  ...
  function finish(config, s) {
    stopTimer(Object.keys(MAP_ENGINE_CONFIG).find(key => MAP_ENGINE_CONFIG[key] === config));

    const result = saveBestRun(config, {
      correct: s.correct,
      wrong: s.wrong,
      time: Date.now() - s.startTime
    });

    updateBestHome(Object.keys(MAP_ENGINE_CONFIG).find(key => MAP_ENGINE_CONFIG[key] === config));
    document.body.classList.remove("map-playing");
...
function startMapQuiz(key) {
  return MapQuizEngine.start(key);
}
...
function wireMapEngine() {
  startEuropeMapQuiz = () => startMapQuiz("europe");
  startAsiaMapQuiz = () => startMapQuiz("asia");
  startAfricaMapQuiz = () => startMapQuiz("africa");
  startSouthAmericaMapQuiz = () => startMapQuiz("southAmerica");
  startNorthAmericaMapQuiz = () => startMapQuiz("northAmerica");

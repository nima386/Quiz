...
function renderArenaDuels(store = loadArenaStore()) {
  ...
  list.innerHTML = store.duels.map(duel => {
    ...
    const status = duel.status === "waiting" ? "Bereit zum Spielen" : "Abgeschlossen";
...
function sendArenaChallenge() {
  ...
  const playerScore = createArenaPlayerScore(gameType, category);
  const opponentScore = Math.max(1000, Math.min(createArenaOpponentScore(selectedArenaFriendId, gameType, category), playerScore - 120));
  ...
  duel.winner = duel.opponentScore >= duel.challengerScore ? "you" : "friend";

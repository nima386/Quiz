...
const ARENA_STORAGE_KEY = "arenaStoreV1";
...
const ARENA_GAME_LABELS = {
  ...
  asia: "Asien Map",
  africa: "Afrika Map",
  southAmerica: "Suedamerika Map",
  northAmerica: "Nordamerika Map",
  quiz: "Bauzeichner-Quiz",
...
function createArenaStore() {
  return {
    ...
    friends: [
      {
        ...
        losses: 4,
        history: [
          { game: "Europa Map", winner: "you", yourScore: 9240, friendScore: 8710 },
          { game: "Asien Map", winner: "friend", yourScore: 6800, friendScore: 7100 },
          { game: "Afrika Map", winner: "you", yourScore: 9300, friendScore: 8800 }
        ]
      },
      {
        ...ARENA_DISCOVERY[1],
        ...
        losses: 5,
        history: [
          { game: "Nordamerika Map", winner: "friend", yourScore: 7520, friendScore: 8040 },
          { game: "Europa Map", winner: "you", yourScore: 8910, friendScore: 8520 }
        ]
      }
    ],
    requests: [
      { id: "req-lina", from: ARENA_DISCOVERY[2], createdAt: Date.now() - 3600000 }
    ],
    duels: [
      {
        id: "duel-seed-1",
        friendId: "friend-ahmad",
        gameType: "europe",
        mode: "async",
        status: "waiting",
    ...
    daily: [
      { name: "Ahmad", score: 8420 },
      { name: "Geo King", score: 7990 }
    ]
  };
}
...
function renderArenaRequests(store = loadArenaStore()) {
  ...
  list.innerHTML = store.requests.map(request => `
    <article class="arena-request-card">
      <img src="${request.from.avatar}" alt="">
      <div>
        <strong>${request.from.name} moechte dich als Freund hinzufuegen.</strong>
        <span>${request.from.username} · Level ${request.from.level}</span>
      </div>
...
function renderArenaDuels(store = loadArenaStore()) {
  ...
  list.innerHTML = store.duels.map(duel => {
    const friend = store.friends.find(item => item.id === duel.friendId) || ARENA_DISCOVERY.find(item => item.id === duel.friendId);
    const status = duel.status === "waiting" ? "Score verborgen · 24h Zeit" : "Abgeschlossen";

    return `
      <article class="arena-duel-card">
        <div>
          <span>${ARENA_MODE_LABELS[duel.mode]}</span>
          <h3>${ARENA_GAME_LABELS[duel.gameType]} gegen ${friend?.name || "Freund"}</h3>
          <p>${status}</p>
        </div>
        <button onclick="playArenaDuel('${duel.id}')">${duel.status === "waiting" ? "Jetzt spielen" : "Ansehen"}</button>
      </article>
    `;
  }).join("");
}
...
function searchArenaFriend() {
  ...
  const found = ARENA_DISCOVERY.find(friend =>
    friend.username.toLowerCase() === query ||
    friend.username.toLowerCase().includes(query.replace("@", "")) ||
    friend.code.toLowerCase() === query
  );

  if (!found) {
    result.innerHTML = `<div class="arena-search-result empty">Kein Profil gefunden.</div>`;
    return;
  }

  const alreadyFriend = store.friends.some(friend => friend.id === found.id);
  const requested = store.requests.some(request => request.from.id === found.id);

  result.innerHTML = `
    <div class="arena-search-result">
      <img src="${found.avatar}" alt="">
      <div>
        <strong>${found.name}</strong>
        <span>Level ${found.level} · ${found.title} · ${found.accuracy}% Accuracy</span>
      </div>
      <button ${alreadyFriend || requested ? "disabled" : ""} onclick="sendArenaFriendRequest('${found.id}')">
...
function sendArenaFriendRequest(friendId) {
  const store = loadArenaStore();
  const found = ARENA_DISCOVERY.find(friend => friend.id === friendId);
  if (!found) return;

  if (!store.requests.some(request => request.from.id === friendId) && !store.friends.some(friend => friend.id === friendId)) {
    store.requests.push({ id: `req-${Date.now()}`, from: found, createdAt: Date.now() });
    saveArenaStore(store);
  }

  renderArena();
  searchArenaFriend();
  showIsland("Freundesanfrage erstellt", "success");
}
...
function sendArenaChallenge() {
  ...
  const duel = {
    ...
    gameType: document.getElementById("challengeGameType").value,
    mode: document.getElementById("challengeMode").value,
    status: "waiting",
    challengerScore: createArenaScore(),
    opponentScore: null,
    createdAt: Date.now()
  };
  ...
  renderArena();
  showIsland("Challenge gesendet", "success");
}

function createArenaScore() {
  return Math.floor(6800 + Math.random() * 2800);
}
...
function playArenaDuel(duelId) {
  ...
  if (duel.status === "waiting") {
    duel.opponentScore = createArenaScore();
    duel.status = "finished";
    const youWin = duel.opponentScore >= duel.challengerScore;
    duel.winner = youWin ? "you" : "friend";
    updateArenaRivalry(store, duel, youWin);
    saveArenaStore(store);
  }
...
function updateArenaRivalry(store, duel, youWin) {
  ...
  friend.history.unshift({
    game: ARENA_GAME_LABELS[duel.gameType],
    winner: youWin ? "you" : "friend",
    yourScore: duel.opponentScore,
    friendScore: duel.challengerScore
  });
}
...
function renderDuelResult(duelId) {
  ...
  const yourScore = duel.opponentScore || 0;
  const friendScore = duel.challengerScore || 0;
  ...
  document.getElementById("duelResultBox").innerHTML = `
    <div class="duel-winner-card ${youWin ? "victory" : "defeat"}">
      <span>${ARENA_GAME_LABELS[duel.gameType]} · ${ARENA_MODE_LABELS[duel.mode]}</span>
